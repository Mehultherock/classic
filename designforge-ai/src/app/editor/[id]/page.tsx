...
function EditorPage() {
        ...
        if (params?.id && params.id !== "new") {
          ...
          if (imageParamRef.current) {
            ...
            elements.push({
              ...
              visible: true,
              properties: {
                content: "Double-click above text to customize",
                fontFamily: "Inter",
                fontSize: 16,
                fontWeight: 400,
                color: "#ffffff",
    ...
    async (project: Project) => {
      try {
        ...
        if (project.id.startsWith("new_") && saved.id) {
          ...
          router.replace(`/editor/${saved.id}`, { scroll: false });
        }
      } catch (err) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        if (selectedElements.length > 0 && document.activeElement?.tagName !== "INPUT" && document.activeElement?.tagName !== "TEXTAREA") {
          const el = currentProject?.elements.find((e) => e.id === selectedElements[0]);
          if (el) {
            pushHistory([...(currentProject?.elements || [])]);
            useProjectStore.getState().removeElement(selectedElements[0]);
            clearSelection();
          }
        }
      }
  ...
  }, [selectedElements, currentProject, pushHistory, clearSelection]);
